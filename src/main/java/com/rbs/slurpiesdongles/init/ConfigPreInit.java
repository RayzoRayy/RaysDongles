package com.rbs.slurpiesdongles.init;

import net.minecraft.item.Item.ToolMaterial;
import net.minecraft.item.ItemArmor.ArmorMaterial;


/**
 * Created by Consular on 7/20/2017.
 */
public class ConfigPreInit {

    //Armor
    public static ArmorMaterial rubyArmor;
    public static ArmorMaterial sapphireArmor;
    public static ArmorMaterial topazArmor;
    //Battle Axe Materials
    public static ToolMaterial diamondBattleaxeMaterial;
    public static ToolMaterial emeraldBattleaxeMaterial;
    public static ToolMaterial goldBattleaxeMaterial;
    public static ToolMaterial ironBattleaxeMaterial;
    public static ToolMaterial rubyBattleaxeMaterial;
    public static ToolMaterial sapphireBattleaxeMaterial;
    public static ToolMaterial stoneBattleaxeMaterial;
    public static ToolMaterial topazBattleaxeMaterial;
    public static ToolMaterial woodenBattleaxeMaterial;
    //Paxel Materials
    public static ToolMaterial diamondPaxelMaterial;
    public static ToolMaterial emeraldPaxelMaterial;
    public static ToolMaterial goldPaxelMaterial;
    public static ToolMaterial ironPaxelMaterial;
    public static ToolMaterial stonePaxelMaterial;
    public static ToolMaterial rubyPaxelMaterial;
    public static ToolMaterial sapphirePaxelMaterial;
    public static ToolMaterial topazPaxelMaterial;
    public static ToolMaterial woodenPaxelMaterial;
    //Sword Materials
    public static ToolMaterial emeraldSwordMaterial;
    public static ToolMaterial rubySwordMaterial;
    public static ToolMaterial sapphireSwordMaterial;
    public static ToolMaterial topazSwordMaterial;
    //Tool Materials
    public static ToolMaterial emeraldMaterial;
    public static ToolMaterial rubyMaterial;
    public static ToolMaterial sapphireMaterial;
    public static ToolMaterial topazMaterial;
    public static ToolMaterial vmpickMaterial;

}
